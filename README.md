...
2. 正常切换过启动器，有目标名为`snow_launcher.exe`的符号链接（文件资源管理器的*详细信息*查看模式下类型显示`.symlink`且，文件大小显示0字节）。

那么您可以为`snow_launcher.exe`创建桌面快捷方式，双击启动就是对应服的启动器（只是这种情况切服需要用bat脚本或调整配置的QS）。
</details>
